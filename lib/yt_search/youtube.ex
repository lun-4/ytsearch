defmodule YtSearch.Youtube do
  ...
  def fetch_subtitles(youtube_id) do
    ...
    with {:ok, subtitles} <- piped_call(:subtitles, &Piped.streams/2, youtube_id, "subtitles") do
      subtitle_count =
        subtitles
      ...
      {:error, _} = v ->
        Logger.error("failed to fetch subtitles: #{inspect(v)}. setting it as not found")
        YtSearch.Subtitle.insert(youtube_id, "notfound", nil)
        nil
    end
  end
