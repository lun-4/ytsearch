...
hello_results=$(curl -A 'UnityWebRequest' -v "http://$HOST/api/v2/hello")
...
check_slot(){
  ...
  quest_request=$(curl -w '%{http_code}' -o /dev/null -A 'stagefright' "http://$HOST/a/2/sl/$slot_id")
  if [ "$quest_request" != "302" ]; then
    echo "expected 302, got $quest_request"
    exit 1
  fi

  redirect_request=$(curl -w '%{http_code}' -o /dev/null -A 'stagefright' "http://$HOST/a/2/sr/$slot_id")
  if [ "$redirect_request" != "302" ]; then
    echo "expected 302, got $redirect_request"
    exit 1
  fi

  unity_request=$(curl -w '%{http_code}' -o /dev/null -A 'UnityWebRequest' "http://$HOST/a/2/sl/$slot_id")
  if [ "$unity_request" != "200" ]; then
    echo "expected 200, got $unity_request"
    exit 1
  fi

  any_request=$(curl -w '%{http_code}' -o /dev/null "http://$HOST/a/2/sl/$slot_id")
...
sleep 4
result=$(curl -A 'UnityWebRequest' -v -G "http://$HOST/a/2/s" --data-urlencode "q=$search_param")
